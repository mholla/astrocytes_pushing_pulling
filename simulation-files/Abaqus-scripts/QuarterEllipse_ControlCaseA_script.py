...
def Create_Material(ModelName, Materials): 
    ...
    mdb.models[ModelName].materials['WHIT'].UserMaterial(mechanicalConstants=(shear_w,lambda_w,Gctx,scaled_thresh,smoothening,GwByGgr,progen_grow_time,N_gyri,gamma_push,reduction,push_grow_time))
...
for j in range(len(gamma_push_list)):
    ...
    # ======================================================
    # Material Parameters (TABLE 1 in manuscript)
    # ======================================================
    CorticalMaterial = 'GRAY'
    SubCortMaterial = 'WHIT'
    GwByGgr = 1.0 # gamma parameter for pushing effect due to progenitors
    progen_grow_time = 0.000001 # arbitrarily small time so that there is no GZ expansion and GM grows instantly.
    smoothening = 10.0 # For smoothening in heaviside function, \bar{\alpha}, not used in control case but since the same vumat is used as the push case, this parameter remains.
    scaled_thresh = 0.415 # parameter for phi growth rate function -> This is used in vumat_push.f to calculate scaled_thresh which is the same as \bar{\delta} and \delta in Table 1. Not used as above.
    shear_g = 0.002 # N/mm2 shear modulus for cortex (Gray matter)
    shear_w = 0.001 # N/mm2  shear modulus for subcortex (White matter)
    lambda_w = 0.1 # N/mm2 
    lambda_g = 0.2 # N/mm2 
    Gctx = 1.5 # growth rate constant for cortex across 0 to 1 simulation time, which translates to 0.084/postnatal day
    N_gyri = 4.0 # Number of proliferation zones
    gamma_push = gamma_push_list[j] # gamma parameter for pushing effect due to astrocytes but not used here.
    ...
    # ======================================================
        # Model
    # ======================================================
    ModelName = 'QuarterEllipse_controlA_%d' %(int(gamma_push_list[j]))
    PartName = 'Part-1'
    InstanceName = 'Part-1'
    JobName = 'QuarterEllipse_controlA_%d' %(int(j+1))
    # ======================================================
    # Call Functions
    # ======================================================
